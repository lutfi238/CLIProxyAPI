...
export async function activate(context: vscode.ExtensionContext) {
	...
	*/
	...
	// Register the exportAuthToken command handler for CLIProxyAPI integration
	context.subscriptions.push(
		vscode.commands.registerCommand("cline.exportAuthToken", async () => {
			try {
				// Get the auth data from SecretStorage
				const authDataString = await context.secrets.get("cline:clineAccountId")

				if (!authDataString) {
					vscode.window.showWarningMessage("No Cline authentication found. Please log in first.")
					return
				}

				// Parse the auth data to get the refresh token
				const authData = JSON.parse(authDataString)
				const refreshToken = authData.refreshToken

				if (!refreshToken) {
					vscode.window.showWarningMessage("No refresh token found in auth data.")
					return
				}
				...
				vscode.window.showInformationMessage("Cline refresh token copied to clipboard! Use this with CLIProxyAPI.", "OK")

				console.log("Cline auth token exported successfully")
			} catch (error) {
				console.error("Failed to export auth token:", error)
				vscode.window.showErrorMessage(`Failed to export auth token: ${error}`)
			}
		}),
	)
